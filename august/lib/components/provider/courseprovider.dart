...
/* ---------------------below is for any other thing------------------------ */
  ...
  void addCourseToCurrentTimetable(ScheduleList course) {
    print('Adding course to timetable...');

    // Ensure the _selectedCoursesData list has enough lists to cover the current page index
    while (_selectedCoursesData.length <= _currentPageIndex) {
      _selectedCoursesData.add([]); // Add an empty list if necessary
    }

    // Now we can safely add the course to the current page.
    _selectedCoursesData[_currentPageIndex].add(course);

    print('Course added: ${course.toJson()}');

    // Optional: Print out the current timetable for debugging
    List<ScheduleList> currentPage = _selectedCoursesData[_currentPageIndex];
    print('Current page after adding course:');
    for (var schedule in currentPage) {
  ...
  //여기서부터 editpage에 사용할거
  void addCoursetoCurrent(ScheduleList course, int timetableIndex) {
    if (timetableIndex >= 0 && timetableIndex < _selectedCoursesData.length) {
      _selectedCoursesData[timetableIndex].add(course);
      print('Current Timetable:');
      for (var c in selectedCoursesData[timetableIndex]) {
  ...
  void addCourestoEdit(ScheduleList course) {
    print('Adding course to timetable...');
